...
export const ProfileBanner = ({ user }: ProfileBannerProps) => {
  return (
    <div className="relative overflow-hidden rounded-[1.8rem] sm:rounded-[2.8rem] bg-gradient-to-br from-primary/10 via-background to-secondary/10 border border-border/40 p-6 sm:p-12 shadow-2xl shadow-primary/5">
      {/* Cinematic Overlays */}
      ...
      <div className="relative flex flex-col sm:flex-row items-center gap-6 sm:gap-10">
        <motion.div 
          ...
          className="w-24 h-24 sm:w-40 sm:h-40 rounded-[2rem] sm:rounded-[3rem] bg-background shadow-2xl shadow-primary/10 flex items-center justify-center border-4 sm:border-8 border-background group hover:rotate-3 transition-transform duration-700"
        >
          <div className="w-full h-full rounded-[1.6rem] sm:rounded-[2.5rem] bg-primary/5 flex items-center justify-center border border-primary/10">
            <span className="text-4xl sm:text-6xl font-black text-primary/40 group-hover:text-primary transition-colors duration-500">
        ...
        <div className="text-center sm:text-left space-y-3 sm:space-y-4">
          <div className="space-y-1">
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-3 sm:gap-4">
              <h2 className="text-2xl sm:text-5xl font-black tracking-tighter text-heading leading-tight">
                {user?.name}
              </h2>
